...
export default function CheckoutPage() {
    ...
    const calculateSubtotal = () => {
        if (isBuyNow && buyNowItem) {
            const price = buyNowItem.salePrice
                || buyNowItem.product?.salePrice
                || 0;
        ...
        if (cart?.items?.length > 0) {
            return cart.items.reduce((sum, item) => {
                const price = item.salePrice || item.price || item.product?.salePrice || 0;
            ...
            const orderData = {
                ...
                ...(isBuyNow && buyNowItem && {
                    items: [{
                        ...
                        salePrice: buyNowItem.salePrice || buyNowItem.product?.salePrice,
                    }],
                }),
                customerInfo: {
    ...
    if (!pageReady || (isBuyNow && !buyNowItem)) {
        return (
                                        ...
                                        <div key={item._id || `${item.product?._id}-${index}`} className="flex gap-3">
                                            ...
                                            <div className="flex-1">
                                                ...
                                                <p className="text-sm font-semibold">
                                                    {/* ✅ FIX 7: safe price access */}
                                                    ${((item.salePrice || item.product?.salePrice || 0) * item.quantity).toFixed(2)}
                                                </p>
                                            </div>
                                        </div>
                                    ))}
